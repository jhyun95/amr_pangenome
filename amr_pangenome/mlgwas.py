...
def grid_search_svm_rse(df_features, df_labels, df_aro, drug,
    ...
    block_features=None, n_jobs=1, cv_jobs=1, seed=1):
    '''
    ...
    fixed_parameters : dict
        ...
        or if preceded by "SVM_", kwargs for sklearn.svm.LinearSVC.
    block_features : list
        If compressed features are being used, the list generated by 
        compress_features() for mapping feature blocks to original features.
...
def save_block_features(block_features, filename):
    ''' Saves block_features list of lists from compress_features() to TSV '''
    with open(filename, 'w+') as f:
        for b, block in enumerate(block_features):
            block_name = 'B' + str(b)
            f.write(block_name + '\t' + ';'.join(block) + '\n')

            
def load_block_features(filename):
    ''' Loads block_features from file in the format outputted by compress_features() '''
    block_features = []
    with open(filename, 'r') as f:
        for line in f:
            block_name, block = line.strip().split('\t')
            block_features.append(block.split(';'))
...
def binarize_amr_table(df_amr):
    ...
    def binarize_sir(sir):
        ...
        if sir in ['Susceptible', 'Susceptible-dose dependent']:
            ...
            return 1
        elif sir in [np.nan, 'Not defined', 'nan', 'NaN'] or pd.isnull(sir):
            return np.nan
        else:
            print 'Unknown SIR:', sir, type(sir)
            return np.nan
